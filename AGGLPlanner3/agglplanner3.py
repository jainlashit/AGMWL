...
import sys, traceback, os, re, threading, time, string, math, copy, pickle
import collections, imp, heapq
import datetime
sys.path.append('/usr/local/share/agm/')

import xmlModelParser as xmlModelParser
from AGGL import *
import inspect

from generateAGGLPlannerCode3 import *
from parseAGGL import AGMFileDataParsing
import agglplanchecker3 as agglplanchecker
from agglplanchecker3 import *
from agglplannerplan3 import *
...
if __name__ == '__main__': # program domain problem result
	...
	#with PyCallGraph(output=graphviz):
		...
		if len(sys.argv)<6:
			print 'Usage\n\t', sys.argv[0], ' domain.aggl activeRules.py init.xml target.xml.py prb_distrb.prb [result.plan] [input_hierarchical.plan]'
			...
			threshData = sorted(pickle.load(open(sys.argv[5])))
			...
			p = AGGLPlanner(domainAGM, domainRuleSet, initPath, targetCode, threshData, '', dict(), [], resultFile)
			p.run()
...
##
	...
	def startThreadedWork(self, ruleMap, triggerMap, lock=None, i=0, threadPoolStatus=None):
		...
		while True:
			...
			# Loop shall run for one chunk at a time
			while True:
				...
				try:
					# We take the head of the openNodes list: the first open node.
					headNode = self.openNodes.heapqPop() # P O P   POP   p o p   pop # P O P   POP   p o p   pop
					head = headNode[1]
					if threadPoolStatus:
						threadPoolStatus.lock()
						threadPoolStatus[i] = True # We say that the thread i is active.
				...
				if head.cost > maxCost:
					...
					return
				if verbose>5: print 'Expanding'.ljust(5), head
				for k in threshData[0:int(len(threshData) * chunkSize[chunkNumber])]:

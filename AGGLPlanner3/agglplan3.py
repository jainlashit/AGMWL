...
"""@package agglplan
    ...
    MODE USE:	agglplan3 gramatica.aggl init.xml target.xml
...
if __name__ == '__main__': # program domain problem result
	...
	if len(sys.argv)<5:
		print 'Usage\n\t', sys.argv[0], ' domain.aggl init.xml target.xml store.data [result.plan]'
	else:
		...
		## pickled trained data
		trainFile = sys.argv[4]
		## we store probability distribution in threshFile 
		threshFile = "prb_distrb.prb"
		Generate.get_distrb(worldFile, targetFile, trainFile, threshFile)
